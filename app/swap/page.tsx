...
const VENUES = [
  ...
  },
  {
    ...
    letter: 'R',
    // Raydium brand: white card, dark navy logo bg
    cardCls: 'bg-white border border-gray-100 shadow-sm hover:border-gray-200',
    logoBg: 'bg-[#141415]',
      ...
      <div className="max-w-[520px] mx-auto px-4 py-8 space-y-4">
        ...
        <div className="rounded-2xl border border-gray-100 bg-white shadow-sm px-5 py-4 space-y-3">
          ...
          <div className="flex items-center gap-2 pt-1 border-t border-gray-100">
            <div className="text-[10px] text-gray-400 uppercase tracking-widest flex-shrink-0">CA</div>
            <a
              href={`https://solscan.io/token/${MINT}`}
              target="_blank"
              rel="noopener noreferrer"
              className="font-mono text-xs text-gray-500 hover:text-green-600 transition-colors truncate flex-1"
            >
              {MINT}
            </a>
            <button
              ...
              className="flex-shrink-0 text-gray-300 hover:text-green-600 transition-colors p-1 rounded-lg hover:bg-green-50"
            >
              {copied
        ...
        <div className="space-y-2.5">
          ...
          {VENUES.map(({ name, sub, logo, letter, cardCls, logoBg, href, primary, tag }) => (
            <a
              key={name}
              href={href}
              target="_blank"
              rel="noopener noreferrer"
              className={`flex items-center gap-4 rounded-2xl px-4 py-4 transition-all active:scale-[0.99] group ${cardCls}`}
            >
              <PlatformLogo src={logo} letter={letter} bg={logoBg} />
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span className={`font-bold text-sm ${primary ? 'text-white' : 'text-gray-900'}`}>{name}</span>
                  {tag && (
                    <span className="text-[10px] font-bold bg-white/20 text-white px-2 py-0.5 rounded-full tracking-wide">{tag}</span>
                  )}
                </div>
                <div className={`text-xs mt-0.5 ${primary ? 'text-white/60' : 'text-gray-400'}`}>{sub}</div>
              </div>
              <div className={`flex items-center gap-1.5 font-semibold text-sm ${primary ? 'text-white/80' : 'text-gray-500 group-hover:text-gray-700'}`}>
                SOL → SMELT
                <ExternalArrow />
              </div>
            </a>
